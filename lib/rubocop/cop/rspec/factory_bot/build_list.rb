  ...
  module Cop
    module RSpec
      module FactoryBot
        ...
        #
        # @example `EnforcedStyle: build_list`
        #   # bad
        #   3.times { build :user }
        #
        #   # good
        ...
        class BuildList < Base
          ...
          MSG_BUILD_LIST = 'Prefer build_list.'
          ...
          def_node_matcher :factory_list_call, <<-PATTERN
            (send {(const nil? {:FactoryGirl :FactoryBot}) nil?} :build_list (sym _) (int $_) ...)
          PATTERN

          def on_block(node)
            return unless style == :build_list
            return unless n_times_block_without_arg?(node)
            return unless contains_only_factory?(node.body)

            add_offense(node.send_node, message: MSG_BUILD_LIST) do |corrector|
          ...
          class TimesCorrector
            ...
            def generate_n_times_block(node)
              ...
              replacement += format_method_call(node, 'build', arguments)
          ...
          class CreateListCorrector
            ...
            def call_with_block_replacement(node)
              ...
              replacement += format_method_call(block, 'build_list', arguments)
